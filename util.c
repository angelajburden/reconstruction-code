...
void splint(double xa[], double ya[], double y2a[], int n, double x, double *y)
{
  ...
  h=xa[khi]-xa[klo];
  if (h == 0.0) err_handler("Bad xa input to routine splint");
  a=(xa[khi]-x)/h;
  b=(x-xa[klo])/h;
  *y=a*ya[klo]+b*ya[khi]+((a*a*a-a)*y2a[klo]+(b*b*b-b)*y2a[khi])*(h*h)/6.0;
}

// first derivative of spline fit
void splind(double xa[], double ya[], double y2a[], 
	    int n, double x, double *dydx)
{
  int klo,khi,k;
  double h,b,a;
  ...
  h=xa[khi]-xa[klo];
  if (h == 0.0) err_handler("Bad xa input to routine splind");
  a=(xa[khi]-x)/h;
  b=(x-xa[klo])/h;
